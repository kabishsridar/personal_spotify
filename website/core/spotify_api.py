...
class SpotifyAPI:
    ...
    def search_tracks(self, query, limit=20, offset=0):
        ...
        if not self.sp:
            return self._search_youtube(query, limit, offset)
        
        try:
            results = self.sp.search(q=query, limit=limit, offset=offset, type='track')
            tracks = []
    ...
    def _search_youtube(self, query, limit=10, offset=0):
        """Search YouTube for metadata and streamable IDs directly."""
        import yt_dlp
        total_fetch = offset + limit
        ydl_opts = {
            'quiet': True,
            'extract_flat': True,
            'skip_download': True,
        ...
        print(f"Searching Global YouTube Database for: {query} (limit={limit}, offset={offset})...")
        try:
            with yt_dlp.YoutubeDL(ydl_opts) as ydl:
                info = ydl.extract_info(f"ytsearch{total_fetch}:{query}", download=False)
                entries = info.get('entries', [])
                
                # Slice to get the entries corresponding to the offset
                entries_slice = entries[offset:offset+limit]
                
                tracks = []
                for item in entries_slice:
                    yt_id = item['id']
                ...
                print(f"YouTube Search returned {len(tracks)} results.")
                return tracks
        except Exception as e:
